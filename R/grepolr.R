...
function(formula,subjects,data,times,categories,
 ...
 while (ogstop>tol & iterct<=maxiter){
  ...
  # fit GEE
  mod.gee <- suppressWarnings(gee(formula,data=exdata$exdata,id=exdata$exdata$subjects,
  ...
  # monitor
  iterct <- iterct+1
  if (iterct==1){monitor.crit <- crit} else {monitor.crit <- append(monitor.crit,crit)}
  if(corstr!="independence"){
   if (iterct==1){monitor.alpha <- alpha} else {monitor.alpha <- append(monitor.alpha,alpha)}
  }
  if(fixed==FALSE & corstr!="independence"){
   if (iterct==1){monitor.grad1 <- pupdate$gVB} else {monitor.grad1 <- append(monitor.grad1,pupdate$gVB)}
   if (iterct==1){monitor.grad2 <- pupdate$ggVB} else {monitor.grad2 <- append(monitor.grad2,pupdate$ggVB)}
  }
  ...
  # print monitor
  cat("\n")
  cat(" REPOLR: 2013-03-20 version 2.0","\n")
  cat(" Working correlation model: ",corstr,"\n")
  cat(" Convergence monitor","\n")
  cat("\n")
  ...
  for (cmont in 1:iterct){
   if (corstr=="independence"){
     cat(format(round(cmont,0),digits=5,trim=TRUE,justify="right",scientific=FALSE,nsmall=0,width=8),
     "\t",format(round(monitor.crit[cmont],5),digits=5,trim=TRUE,justify="right",scientific=FALSE,nsmall=5,width=8),
    ...
    cat(format(round(cmont,0),digits=5,trim=TRUE,justify="right",scientific=FALSE,nsmall=0,width=8),
     "\t",format(round(monitor.crit[cmont],5),digits=5,trim=TRUE,justify="right",scientific=FALSE,nsmall=5,width=8),
     "\t",format(round(monitor.alpha[cmont],5),digits=5,trim=TRUE,justify="right",scientific=FALSE,nsmall=5,width=8),
     "\t",format(round(monitor.grad1[cmont],5),digits=5,trim=TRUE,justify="right",scientific=FALSE,nsmall=5,width=8),
     "\t",format(round(monitor.grad2[cmont],5),digits=5,trim=TRUE,justify="right",scientific=FALSE,nsmall=5,width=8),"\n")
    }
    if (fixed==TRUE){
  ...
  if (pupdate$ggVB>0){
   ...
   if (po.test!=TRUE){cat("\n")}
   cat(" Correlation parameter:",round(alpha,3),"\n")
  } else {
